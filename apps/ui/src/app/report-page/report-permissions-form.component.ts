...
@Component({
  ...
  standalone: true,
  selector: 'app-report-permissions-form',
  template: `
...
export class ReportPermissionsFormComponent implements OnInit {
  ...
  loadPermissions() {
    this.http
      ...
      >(`/api/v1/report-permission/${this.reportId}`)
      .pipe(
  ...
  addPermission() {
    ...
    this.http
      .post(`/api/v1/report-permission/add/${this.reportId}`, { login, role })
      .subscribe(() => {
        this.loadPermissions();
  ...
  onRoleChange(login: string, event: MatSelectChange) {
    const newRole = event.value as
      | 'viewer'
      | 'editor'
      | 'admin';

    this.http
      .patch(`/api/v1/report-permission/update/${this.reportId}`, {
        login,
        role: newRole,
      })
